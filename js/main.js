// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// WAVE ENGINE — runs fixed bg + hero + cta
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    ...
    waves.forEach(w => {
      ...
      for(let x = 0; x <= W; x += 4){
        const y = (w.y * H) + Math.sin(x * w.freq + w.phase) * w.amp
                             + Math.sin(x * w.freq * 2.3 + w.phase * 1.4) * (w.amp * 0.4);
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// CTA WAVE — full-width SVG-driven sine animation
// Uses SVG path updated by rAF — guaranteed full width
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
function initCtaWave(){
  // Replace each canvas#cta-wave and #cta-wave2 with inline SVG
  ['cta-wave','cta-wave2'].forEach(function(id){
    ...
    // Create wave paths — 7 lines at different heights/speeds
    const waves = [];
    const configs = [
    ...
    configs.forEach(function(cfg, i){
      ...
      waves.push({el:path, cfg:cfg, phase: i * Math.PI * 0.8});
    });
    ...
    function animate(){
      ...
      const W = svg.getBoundingClientRect().width || svg.offsetWidth || window.innerWidth;
      const H = svg.getBoundingClientRect().height || svg.offsetHeight || 280;
      waves.forEach(function(w){
        w.phase += w.cfg.speed;
        const y0 = w.cfg.y * H;
        let d = '';
        for(let x = 0; x <= W + 4; x += 4){
          const y = y0 + Math.sin(x * w.cfg.freq + w.phase) * w.cfg.amp
                       + Math.sin(x * w.cfg.freq * 2.1 + w.phase * 1.3) * (w.cfg.amp * 0.35);
...
function wheelGo(n){
  ...
  setTimeout(() => {
    document.querySelectorAll('.wheel-slide.prev').forEach(s => s.className = 'wheel-slide next');
  }, 800);
...
function initLogCanvas(){
  ...
  function draw(){
    requestAnimationFrame(draw);
    // Speed: 0.006 * (1 + 0.08) gives a slight dwell at node 4 before wrapping
    t = (t + 0.006) % 1.08;
    if(!nodeX.length || W < 10) return;
    ctx.clearRect(0, 0, W, H);

    const x0 = nodeX[0], x3 = nodeX[nodeX.length - 1];
    const span = x3 - x0;

    // Track lines — node to node
    for(let i = 0; i < nodeX.length - 1; i++){
      ctx.beginPath();
      ctx.setLineDash([6, 9]);
      ctx.strokeStyle = 'rgba(42,128,204,0.22)';
      ctx.lineWidth = 1.5;
      ctx.moveTo(nodeX[i], CY);
      ctx.lineTo(nodeX[i+1], CY);
      ctx.stroke();
    }
    ctx.setLineDash([]);

    // Node arrival flare — pulses when a comet is near
    nodeX.forEach(nx => {
      [0,1,2].forEach(ci => {
        const p = (t + ci/3) % 1.08;
        ...
        if(dist < 22){
          ...
          ctx.beginPath();
          ctx.arc(nx, CY, 10 * intensity, 0, Math.PI*2);
          ctx.fillStyle = 'rgba(82,168,245,' + (0.18 * intensity) + ')';
          ctx.fill();
        }
      });
    });

    // 3 staggered comets — each travels from node 1 to node 4 completely
    [0,1,2].forEach(ci => {
      const p = (t + ci/3) % 1.08;
      if(p > 1.0) return; // brief invisible gap between cycles
      const absX = x0 + p * span;
      const tailLen = 80;
      const tailX = Math.max(x0, absX - tailLen);

      // Tail — gradient from transparent to bright blue
      const g = ctx.createLinearGradient(tailX, CY, absX, CY);
      g.addColorStop(0,   'rgba(42,128,204,0)');
      g.addColorStop(0.4, 'rgba(82,168,245,0.2)');
      g.addColorStop(1,   'rgba(82,168,245,0.85)');
      ctx.beginPath();
      ctx.strokeStyle = g;
      ctx.lineWidth = 2.8;
      ctx.moveTo(tailX, CY);
      ...
      rg.addColorStop(0.3,  'rgba(82,168,245,0.85)');
      rg.addColorStop(1,    'rgba(42,128,204,0)');
      ctx.beginPath();
      ctx.arc(absX, CY, 14, 0, Math.PI*2);
      ctx.fillStyle = rg;
      ctx.fill();

      // Bright white core
      ctx.beginPath();
      ctx.arc(absX, CY, 3, 0, Math.PI*2);
      ctx.fillStyle = '#fff';
      ctx.fill();
    });
  }
...
function handleForm(e){
  ...
  const data = {
    ...
    interest:  form.inq.value,
    message:   form.msg.value,
  };

  // If Formspree is configured, submit to it
  if(!FORMSPREE_ENDPOINT.includes('YOUR_FORMSPREE_ID')){
    try {
      ...
      if(res.ok){
        ...
        resp.textContent='Thank you — your message has been received. We will respond within one business day.';
      ...
      resp.style.display='block'; resp.className='form-resp err';
      resp.textContent='Something went wrong. Please call us directly at 740.535.8148.';
    }
  } else {
    // Formspree not yet configured — show success for preview
    resp.style.display='block'; resp.className='form-resp ok';
    resp.textContent='Thank you — your message has been received. We will respond within one business day.';
    form.reset(); document.getElementById('ft').value = Date.now();
  }
